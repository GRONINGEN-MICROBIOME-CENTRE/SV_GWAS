...
then
  ...
  cat ${d}/scripts/scripts_${svtype}/metal_header.txt > $metal_script
    ...
    do
        ...
        n=`head -2 ${res_dir}/${svtype}.${cohort}.${sv}.${sv}.glm.linear | tail -1 | awk '{print $9}'`
        all_nsamples+=( $n )
    
        gzip ${res_dir}/${svtype}.${cohort}.${sv}.${sv}.glm.linear
    
        # append the per cohort result location to the metal script
        echo -e "PROCESS\t${res_dir}/${svtype}.${cohort}.${sv}.${sv}.glm.linear.gz\n" >> $metal_script
    ...
    do 
        res_dir=${d}/results/${svtype}/${cohort}/${sv}/
        rm ${res_dir}/${svtype}.${cohort}.${sv}.${sv}.glm.linear.gz
        #tmp
    done
...
#
    ...
    do
        ...
        n=`head -2 ${res_dir}/permutations/${svtype}.${cohort}.${sv}.perm${i}.${sv}.glm.linear | tail -1 | awk '{print $9}'`
        all_nsamples+=( $n )
        gzip ${res_dir}/permutations/${svtype}.${cohort}.${sv}.perm${i}.${sv}.glm.linear 
        
        # append the per cohort result location to the metal script
        echo -e "PROCESS\t${res_dir}/permutations/${svtype}.${cohort}.${sv}.perm${i}.${sv}.glm.linear.gz" >> ${d}/scripts/scripts_${svtype}/metal_per_sv/${sv}.metal.perm${i}.txt
    ...
    do 
        res_dir=${d}/results/${svtype}/${cohort}/${sv}/
        rm ${res_dir}/permutations/${svtype}.${cohort}.${sv}.perm${i}.${sv}.glm.linear.gz
    done
