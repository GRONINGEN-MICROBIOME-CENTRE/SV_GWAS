...
for (c in cohorts){
  ...
  if (svtype == "vSV"){
    ...
    lmm_a_n <- relmatGlmer(as.formula(paste0("sv ~ ABO_A + ", covars)), data = non, relmat = list(ID = kinMat), family = binomial)
    #lmm_a_n <- relmatGlmer(as.formula(paste0("sv ~ ABO_A + ", covars)), data = non, relmat = list(ID = kinMat), family = binomial, calc.derivs = FALSE)
     
    lmm_o <- relmatGlmer(as.formula(paste0("sv ~ ABO_O + ", covars)), data = m , relmat = list(ID = kinMat), family = binomial)
    ...
    lmm_o_n <- relmatGlmer(as.formula(paste0("sv ~ ABO_O + ", covars)), data = non, relmat = list(ID = kinMat), family = binomial)
