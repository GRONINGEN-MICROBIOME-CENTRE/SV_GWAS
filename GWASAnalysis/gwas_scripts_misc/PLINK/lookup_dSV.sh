...
do
    ...
    then
        ...
        z=`head -2 ${res_dir}/${svtype}.${cohort}.${sv}.${snp}.${sv}.glm.logistic | tail -1 | awk '{print $11}'`
        all_zscores+=( $z )
        pval=`head -2 ${res_dir}/${svtype}.${cohort}.${sv}.${snp}.${sv}.glm.logistic | tail -1 | awk '{print $12}'`
        all_pvals+=( $pval )
        ...
        ${res_dir}/${svtype}.${cohort}.${sv}.${snp}.${sv}.glm.logistic | gzip -c > ${res_dir}/${svtype}.${cohort}.${sv}.${snp}.${sv}.glm.logistic.gz
        
        rm ${res_dir}/${svtype}.${cohort}.${sv}.${snp}.${sv}.glm.logistic
        echo -e "PROCESS\t${res_dir}/${svtype}.${cohort}.${sv}.${snp}.${sv}.glm.logistic.gz\n" >> $metal_script
    fi
done
#
# Run meta-analysis
