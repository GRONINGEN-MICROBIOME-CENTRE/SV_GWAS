...
do
    ...
    # Number of samples:
    n=`head -2 ${res_dir}/${type}.${cohort}.${sv}.${sv}.glm.linear | tail -1 | awk '{print $8}'`
    all_nsamples+=( $n )
    
    # format the assoc results_all_summary_stats for METAL: add A1 and A2
    awk '{OFS="\t"}; {if ($6 == $4) {oa=$5}; if ($6 == $5) {oa=$4}; if (NR == 1) {oa="A2"}; {print $1,$2,$3,$6, oa, $7,$8,$9,$10,$11,$12}}' \
    ${res_dir}/${type}.${cohort}.${sv}.${sv}.glm.linear | gzip -c > ${res_dir}/${type}.${cohort}.${sv}.${sv}.glm.linear.gz
    reformat_returncode=$?

    rm ${res_dir}/${type}.${cohort}.${sv}.${sv}.glm.linear

    echo -e "PROCESS\t${res_dir}/${type}.${cohort}.${sv}.${sv}.glm.linear.gz\n" >> $metal_script
    ...
    do
        ...
        # format the assoc results_all_summary_stats for METAL: add A1 and A2
        awk '{OFS="\t"}; {if ($6 == $4) {oa=$5}; if ($6 == $5) {oa=$4}; if (NR == 1) {oa="A2"}; {print $1,$2,$3,$6, oa, $7,$8,$9,$10,$11,$12}}'  \
        ${res_dir}/permutations/${type}.${cohort}.${sv}.perm${i}.${sv}.glm.linear | gzip -c > ${res_dir}/permutations/${type}.${cohort}.${sv}.perm${i}.${sv}.glm.linear.gz
        reformat_returncode=$?

        rm ${res_dir}/permutations/${type}.${cohort}.${sv}.perm${i}.${sv}.glm.linear

        echo -e "PROCESS\t${res_dir}/permutations/${type}.${cohort}.${sv}.perm${i}.${sv}.glm.linear.gz" >> /data/umcg-tifn/SV/SV_GWAS/scripts_all_summary_stats/vSV/metal_per_sv/${sv}.metal.perm${i}.txt
    done
done
